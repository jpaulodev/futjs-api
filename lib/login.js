...
class Login {
  ...
  async _getPersonas() {
    ...
    try {
      response = await this.defaultRequest.get(`https://${this.futHost}/ut/game/fifa20/user/accountinfo`, {
        qs,
        headers,
        timeout: 5000,
        json: true
  ...
  async _getUserInfo() {
    ...
    //userinfo
    const userInfoResponse = await this.defaultRequest.get(`https://${this.futHost}/ut/game/fifa20/usermassinfo`, {
      headers,
      json: true
    });
    ...
    //settings
    const settingsResponse = await this.defaultRequest.get(`https://${this.futHost}/ut/game/fifa20/settings`, {
      qs: {
        '': new Date().getTime()
      },
      headers,
